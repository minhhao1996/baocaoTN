...
Class Catalog extends MY_Controller
{
    ...
    private function _del($id, $rediect = true)
    {
        ...
        $catalog_info = $this->catalog_model->get_info_rule(array('parent_id' => $id),'id');
        if($catalog_info){
            $this->session->set_flashdata('message', 'Danh mục ' .$info->name. ' có chứa danh mục con, bạn cần xóa hết các danh mục con trước khi xóa danh mục');
            if($rediect) {
                redirect(admin_url('catalog'));
            }else{
                return false;
        ...
        {
            //tạo ra nội dung thông báo
            $this->session->set_flashdata('message', 'Danh mục '.$info->name.' có chứa sản phẩm');
            if($rediect)
            {
                redirect(admin_url('catalog'));
            }else{
